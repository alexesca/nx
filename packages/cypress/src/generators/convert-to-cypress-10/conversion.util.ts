...
export default defineConfig(${inspect(cypressConfigs.cypressConfigTs)})
  ...
  // TODO(caleb): only do this if the project changed or does it matter?
...
Check out the Cypress migration guide for assistance https://docs.cypress.io/guides/references/migration-guide`
    );
    return {
      cypressConfigPathJson,
      cypressConfigPathTs,
...
export function updateProjectPaths(
  ...
  // TODO(caleb): should we still rename the default support/index.ts file
  //  even if it's not being used to prevent from getting out of sync?
